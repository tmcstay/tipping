-- Adds "movement since the previous completed stage" to the leaderboard.
-- No rank-history/snapshot table exists anywhere in this schema -
-- get_grandtour_leaderboard() computes `rank` live, stateless, every call.
-- Rather than introducing a stored snapshot (which would need to be
-- populated by a new trigger/job tied into scoring - out of scope here,
-- and this task explicitly must not touch scoring), this derives the
-- "previous" standing on demand: the single most-recently-scored stage
-- across the whole leaderboard is identified, and each user's own total is
-- reduced by their score for exactly that one stage (not an arbitrary
-- lookback window). Users who had no scored stage before that one are
-- reported as previous_rank = null (the caller renders this as "New"),
-- never a fabricated number. Read-only, additive - does not modify
-- get_grandtour_leaderboard() or any scoring RPC.
create or replace function public.get_grandtour_leaderboard_with_movement(
...
-- New function name -> defaults to PUBLIC execute grant; lock it down the
-- same way get_grandtour_leaderboard already is.
revoke all on function public.get_grandtour_leaderboard_with_movement(uuid, text)
from public, anon;
grant execute on function public.get_grandtour_leaderboard_with_movement(uuid, text)
to authenticated;
