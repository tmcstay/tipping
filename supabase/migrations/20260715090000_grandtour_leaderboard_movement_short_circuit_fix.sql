-- Fixes a real bug in the previous migration's own fix
-- (20260715080000): Postgres does NOT guarantee left-to-right
-- short-circuit evaluation of a single SQL boolean expression's AND/OR
-- operands (documented Postgres behaviour, not a bug in Postgres itself -
-- see the "Expression Evaluation Rules" section of the Postgres docs).
-- `auth.role() <> 'service_role' and not
-- grandtour_private.can_access_competition(p_competition_id)` could still
-- evaluate the right-hand function call even when the left-hand side was
-- false, and a service-role caller has no USAGE on the grandtour_private
-- schema at all - confirmed by a real local call failing with
-- "permission denied for schema grandtour_private" (42501), not the
-- intended "Authentication is required."/"GrandTour competition access is
-- required." application-level exceptions.
--
-- Fixed by using PL/pgSQL's own `if ... else ... end if` control flow
-- instead of relying on boolean-expression evaluation order - an
-- imperative branch is guaranteed to skip its unexecuted body, which a
-- single SQL expression's operand order is not. Same signature, safe
-- same-OID `create or replace`; every existing authenticated-caller
-- behaviour is unchanged (identical checks still run for them, just via
-- an if/else instead of an inline and/not).
...
begin
  if auth.role() = 'service_role' then
    -- Internal server-side processing (send-stage-results) - no user
    -- session exists to check, and this caller is trusted by construction
    -- (only ever invoked with the service-role key from within our own
    -- Edge Function runtime, never from a browser).
    null;
  else
